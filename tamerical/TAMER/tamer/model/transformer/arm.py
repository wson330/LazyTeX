...
class MaskBatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor) -> Tensor:
        ...
        flat_mask = not_mask.reshape(-1)
        if flat_mask.any():
            flat_x = x.reshape(-1, x.shape[-1])
            indices = torch.nonzero(flat_mask, as_tuple=False).squeeze(1)
            if indices.numel():
                max_index = torch.max(indices)
                if max_index >= flat_x.shape[0]:
                    raise RuntimeError(
                        f"Mask index {max_index.item()} out of range {flat_x.shape[0]}"
                    )
                selected = torch.index_select(flat_x, 0, indices)
                selected = self.bn(selected)
                flat_x.index_copy_(0, indices, selected)
